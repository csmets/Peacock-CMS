    ...
    function displayBlogNavLinks ($id, $cssStyle){
		...
		for ($oi = $id - 1; $oi > 0; $oi--){
			if ($peacock->checkPostIDExistsNoDrafts($oi) == TRUE){
				echo "<td width='50%'><a class='".$cssStyle."' href='blogPost.php?postID=".$oi."'>Older Post</a></td>";
				break;
			}
        }
		...
		for ($ni = $id + 1; $ni <= $MaxID; $ni++){
			if ($peacock->checkPostIDExistsNoDrafts($ni) == TRUE){
				echo "<td width='50%' align='right'><a class='".$cssStyle."' href='blogPost.php?postID=".$ni."'>Newer Post</a></td>";
				break;
			}
        }
        echo "</tr></table>";
