    ...
    if(isset($name)){
        if(!empty($name)){
            ...
            if (($extention == 'jpg' || $extention == 'jpeg' || $extention == 'png' || $extention == 'gif') && ($type=='image/jpeg'||$type=='image/png'||$type=='image/gif')){
                if ($size<=$maxFileSize){
					...
					if ($folder != null){
						...
						header("location:controlpanel.php?message=SUCESS!");
					}
                }
                else {
                	if ($folder != null){
                    	...
                    	$UploadErrorMessage = "location:controlpanel.php?message=Image file exceeds 6mb";
					}
            	...
            	if ($folder != null){
            		...
            		$UploadErrorMessage = "location:controlpanel.php?message=File type must be jpg / jpeg / png / gif";
            	}
                header($UploadErrorMessage);
            }
        }
        	...
        	if ($folder != null){
        		...
        		$UploadErrorMessage = "location:controlpanel.php?message=No file selected"; 
        	}
            header($UploadErrorMessage);   
        }
    }
