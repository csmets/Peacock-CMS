 ...
 * Peacock CMS is under the GNU - General Public License V3. Please read txt file distributed with source for full license information.
 */
	...
	if (isset($subType)){
		...
		if ($subType == 'addCategory'){
		    ...
		    if (isset($categoryName)){
		        ...
		        mysqli_query($db,"INSERT INTO categories (category,icon) VALUES ('$categoryName','$image')");
		        $db->close();
		    }
		...
		if ($subType == 'arrangeCategories'){
		    ...
		    $db = $sqlconnect->connectTo();
				...
				foreach ($menu as $key => $value){
					$key = $key + 1;
					$sendToDatabase = "UPDATE categories SET categoryOrder='$key' WHERE id='$value'";
		...
		elseif ($subType == 'blogSubmit'){
            ...
            $peacock->storeToFile('/peacock/backups/posts/postBackup-'.$postID.'.json',$contentArray, 'JSON');
		...
		elseif ($subType == 'changePassword'){
		    ...
		    if ($password != null && $retypepassword != null && $userID != null){
		        if ($password == $retypepassword){
		            ...
		            $sqlconnect = new Connectdb;
		            $db = $sqlconnect->connectTo();
		            mysqli_query($db,"UPDATE users SET password='$password' WHERE username='$userID'");
		            $db->close();
		...
		elseif ($subType == 'changeAvatar'){
		    ...
		    if ($avatar != nulll){
		        ...
		        $db = $sqlconnect->connectTo();
				if ($avatar == 'none'){
					mysqli_query($db,"UPDATE users SET profileimg='' WHERE username='$userID'");
				}else{
					mysqli_query($db,"UPDATE users SET profileimg='$avatar' WHERE username='$userID'");
				}
		        $db->close();
      ...
      if ($isDraft == 'yes'){
          $peacock->storeToFile('/peacock/drafts/pages/pageDraft-'.$pageId.'.json',$draftContentArray, 'JSON');

          $sendToDatabase = "UPDATE pages SET draft='$isDraft' WHERE id='$pageId'";

      }else{
          $SQLBodyContents = addslashes($getBodyContents);
					if ($peacock->getPageStatus($pageId) == 'draft' && $isDraft == 'no'){
						$sendToDatabase = "UPDATE pages SET bodycontent='$SQLBodyContents', draft='$isDraft', pagename='$pageTitle', status='active' WHERE id='$pageId'";
					}else{
      ...
      mysqli_query($db,$sendToDatabase) or die(mysqli_error($db));
		...
		//  Default Edit Page
		//=============================================================
		elseif ($subType == 'defaultEditPage'){
      ...
      $SQLBodyContents = addslashes($getBodyContents);
			$sendToDatabase = "UPDATE pages SET bodycontent='$SQLBodyContents', pagename='$pageTitle' WHERE id='$pageId'";
      mysqli_query($db,$sendToDatabase) or die(mysqli_error($db));
			$db->close();
		...
		elseif ($subType == 'editPost'){
            ...
            if ($isDraft == 'yes'){
                ...
                $peacock->storeToFile('/peacock/drafts/posts/postDraft-'.$postId.'.json',$draftContentArray,'JSON');
		...
		elseif ($subType == 'editUser'){
		    ...
		    email='$email', acctype='$accType' WHERE username='$userID'";
				if ($accType == null){
					$sendToDatabase = "
			    UPDATE users SET firstname='$firstName', lastname='$lastName',
			    email='$email' WHERE username='$userID'";
				}
      ...
      $templateContent = addslashes($templateContent);
		...
		elseif ($subType == 'submitPage'){
            ...
            $contentArray['title'] = $pagename;
            $contentArray['body'] = $bodycontent;
            $peacock->storeToFile('/peacock/backups/pages/pageBackup-'.$pageID.'.json',$contentArray,'JSON');
		...
		elseif ($subType == 'siteTheme'){
		    ...
		    if ($themeContents == true){
		    	$SQLRW->readSQL("../view/themes/".$sitetheme."/".$sitetheme.".sql");
		    }
