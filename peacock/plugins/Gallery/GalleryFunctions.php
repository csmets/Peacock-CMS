...
class Gallery{
    ...
    $db = $sqlconnect->connectTo();
		...
		while ($get_data = mysqli_fetch_assoc($query)){
			$array[$index]['id'] = $get_data['id'];
			$array[$index]['FolderName'] = $get_data['FolderName'];
			$array[$index]['FolderImage'] = $get_data['FolderImage'];
			$index++;
		}
		$db->close();
		return $array;
	...
	public function fetchImageList($folder = null){
		$sqlconnect = new Connectdb;
		$db = $sqlconnect->connectTo();
		...
		while ($get_data = mysqli_fetch_assoc($query)){
			...
			if ($folder != null){
				if ($get_data['imageFolder'] == $folder){
					$array[$index]['id'] = $get_data['id'];
					$array[$index]['imageFile'] = $get_data['imageFile'];
					$array[$index]['imageTitle'] = $get_data['imageTitle'];
					$array[$index]['imageDesc'] = $get_data['imageDesc'];
					$array[$index]['lightBox'] = $get_data['lightBox'];
					$array[$index]['hidden'] = $get_data['hidden'];
					$array[$index]['imageFolder'] = $get_data['imageFolder'];
					$index++;
				}
			}else{
				if ($get_data['imageFolder'] == $folder){
	...
	public function fetchImage($id){
		...
		$array['id'] = $get_data['id'];
		$array['imageFile'] = $get_data['imageFile'];
		$array['imageTitle'] = $get_data['imageTitle'];
		$array['imageDesc'] = $get_data['imageDesc'];
		$array['lightBox'] = $get_data['lightBox'];
		$array['hidden'] = $get_data['hidden'];
		$array['imageFolder'] = $get_data['imageFolder'];
		$db->close();
		return $array;
	}
